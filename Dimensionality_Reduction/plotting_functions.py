...
def plotModEigValsVsNullEig(network_modularity_matrix, mean_mins_eig, mean_maxs_eig):
    ...
    plt.axvline(x=mean_maxs_eig, ymin=0, ymax=1, color='red', label='Upper bound')
    plt.axvline(x=0, ymin=0, ymax=1, color='black', linestyle='--')
    plt.scatter(mod_eig_vals, 0.5*np.ones(network_modularity_matrix.shape[0]), color='blue', marker='.', alpha=0.4,label='Data eigenvalues',s=200)
    # plt.xlim([-10*np.ceil(mod_eig_vals.max()/10), 10*np.ceil(mod_eig_vals.max()/10)])
    max_val = np.abs(mod_eig_vals).max()
    xmax = np.ceil(max_val * 1.1)  # 10% buffer
    plt.xlim([-xmax, xmax])
    plt.ylim([0, 1])
...
def plotModEigValsVsNullEigHist(network_modularity_matrix, samples_eig_vals, mean_mins_eig, mean_maxs_eig):
    mod_eig_vals = np.linalg.eigvalsh(network_modularity_matrix)
    plt.hist(samples_eig_vals.flatten(), bins=50, color='black', label='Null Model Eigenvalue Distribution', alpha=0.4)
    plt.yscale('log')  # Set y-axis to log scale
    y_max = plt.ylim()[1]/50.0
    plt.vlines(x=mod_eig_vals, ymin=0, ymax=y_max, color='blue', label='Data Eigen values', alpha=0.4,linewidth=0.75)
    # plt.xlim([-10*np.ceil(mod_eig_vals.max()/10), 10*np.ceil(mod_eig_vals.max()/10)])
    max_val = np.abs(mod_eig_vals).max()
    xmax = np.ceil(max_val * 1.1)  # 10% buffer
    plt.xlim([-xmax, xmax])
    ...
    plt.axvline(x=mean_maxs_eig, ymin=0, ymax=1, color='red', label='Upper bound')
    plt.xlabel('Eigenvalues',fontsize=12)
    plt.ylabel('Num Eigenvalues',fontsize=12)
    plt.legend(fontsize=10)
    ax = plt.gca()
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
