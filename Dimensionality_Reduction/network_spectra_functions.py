...
def getLowDimSpace(modularity_matrix, eig_vals, confidence_level, int_type='CI',alter=1,vary=0):
    """
    ...
    # edits by BSK: the value of alter should be 'alter=1' unless the function is called to check the performance of Spect. Est by varying the number of eigen values above the null model
    ...
    if vary==1:
        if alter <= len(exceeding_upper_bound_inds):
            exceeding_eig_space = mod_eig_vecs[:,exceeding_upper_bound_inds[:alter]]
        else:
            return None, None, [None, None], None, None, [None, None]
    else:
        exceeding_eig_space = mod_eig_vecs[:,exceeding_upper_bound_inds]
        inside_space=mod_eig_vecs[:,min(exceeding_upper_bound_inds)-6:max(exceeding_upper_bound_inds)+1] # the eigen vectore which share null model eigen value range but still could have info
        exceeding_eig_vals= mod_eig_vals[exceeding_upper_bound_inds]
        inside_eig_vals= mod_eig_vals[min(exceeding_upper_bound_inds)-6:max(exceeding_upper_bound_inds)+1]
    below_eig_space = mod_eig_vecs[:,below_lower_bound_inds]
    return below_eig_space, below_lower_bound_inds, [mean_mins_eig, min_confidence_ints], exceeding_eig_space, exceeding_upper_bound_inds, [mean_maxs_eig, max_confidence_ints],inside_space,inside_eig_vals,exceeding_eig_vals


def getEigValsFrmNNR(modularity_matrix, eig_vals, confidence_level, int_type='CI',alter=1,vary=0):
    ...
    return  exceeding_eig_space, exceeding_eig_vals
