...
def getBiggestComponent(pairwise_measure_matrix):
    ...
    if pairwise_measure_matrix.shape != biggest_comp.shape:
        left_out_indices = set(range(pairwise_measure_matrix.shape[0])) - set(keep_indices)
        print(f'One feature dropped. Left out index: {left_out_indices}')
